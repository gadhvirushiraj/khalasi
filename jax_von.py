...
MAX_HORIZONTAL_INFLOW_VELOCITY = 0.15
...
def main():
    ...
    import numpy as np
    # random choice 500 random indices between 2000 - 12000
    random_idx = np.random.choice(range(2000, 12000), 500, replace=False)
    random_states = []

    for iteration_index in tqdm(range(N_ITERATIONS)):
        discrete_velocities_next = update(discrete_velocities_prev)

        if iteration_index in random_idx:
            random_states.append(discrete_velocities_next)
    ...
    plt.close()
    print("Storing the random states, length: ", len(random_states))
    np.save("vortex_init_states.npy", random_states)
